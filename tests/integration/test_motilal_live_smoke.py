"""Live-network smoke test for the Motilal Oswal adapter (spec §9: manually triggered, skips
if network unavailable). Fifth AMC — discovery via a JSON search-documents API found through
a one-time Playwright network capture; combined one-workbook-per-month layout like SBI, but
with different Index-sheet column positions (handled by the shared, column-detecting resolver).
"""
from datetime import date

import httpx
import pytest

from indian_mf_mcp.ingest.amc_adapters.combined_workbook import find_sheet_code
from indian_mf_mcp.ingest.amc_adapters.motilal_oswal import MotilalOswalAdapter
from indian_mf_mcp.ingest.portfolio_ingest import ingest_scheme_portfolios
from indian_mf_mcp.store import repository as repo
from indian_mf_mcp.store.db import get_connection
from indian_mf_mcp.tools.get_fund_portfolio import get_fund_portfolio

SCHEME_ID = "scheme-motilal-flexicap-test"
AMC_ID = "amc-motilal-oswal"
SCHEME_HINT = "Motilal Oswal Flexi Cap Fund"


def _network_available() -> bool:
    try:
        httpx.get("https://www.motilaloswalmf.com/", timeout=10)
        return True
    except Exception:
        return False


@pytest.mark.skipif(not _network_available(), reason="network unavailable")
def test_motilal_end_to_end_live(tmp_path):
    conn = get_connection(tmp_path / "motilal_live.db")
    repo.upsert_amc(conn, AMC_ID, "Motilal Oswal Asset Management Company Limited")
    repo.upsert_scheme(conn, SCHEME_ID, AMC_ID, SCHEME_HINT, "Open Ended Schemes",
                        "Equity Scheme", "Flexi Cap Fund", date.today().isoformat())
    conn.commit()

    adapter = MotilalOswalAdapter()
    stats = ingest_scheme_portfolios(
        conn, adapter, SCHEME_ID, SCHEME_HINT, since=date(2026, 6, 1),
        sheet_resolver=find_sheet_code,
    )
    conn.commit()

    assert stats["documents_found"] > 0
    assert stats["ingested"] > 0
    assert stats["skipped_sheet_not_found"] == 0
    assert stats["skipped_reconciliation_failed"] == 0

    out = get_fund_portfolio(conn, [SCHEME_ID], sections=["holdings", "concentration"])
    data = out[SCHEME_ID]["data"]
    assert len(data["holdings"]["v"]) > 20

    conn.close()
