"""Live-network smoke test for the HSBC Mutual Fund adapter (spec §9: manually triggered,
skips if network unavailable). Discovery needed no API/Playwright: the AMFI-registered
`investor-resources` page turned out not to carry the monthly portfolio XLSX at all — the real
archive lives on `investor-resources/information-library`'s "Fund portfolios" accordion, a
plain HTML table covering every scheme/month in one page load (see hsbc.py's module docstring).
"""
...
SCHEME_ID = "scheme-hsbc-flexicap-test"
AMC_ID = "amc-hsbc"
SCHEME_HINT = "HSBC Flexi Cap Fund"


def _network_available() -> bool:
    try:
        httpx.get("https://www.assetmanagement.hsbc.co.in/", timeout=15)
        return True
    except Exception:
        return False
...
def test_hsbc_end_to_end_live(tmp_path):
    conn = get_connection(tmp_path / "hsbc_live.db")
    repo.upsert_amc(conn, AMC_ID, "HSBC Asset Management (India) Private Limited")
